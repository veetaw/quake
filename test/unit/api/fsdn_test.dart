// TODO:
@Skip("can't mock Dio client with mockito")

import 'dart:async';
import 'dart:io';

import 'package:quake/services/emsc.dart';
import 'package:quake/services/ingv.dart';
import 'package:quake/services/usgs.dart';
...
class _MockedClient extends Mock implements Dio {}
  ...
  group('FSDNews', () {
    ...
    test('fetchData should work correctly with a correct response', () async {
      ...
      when(_client.get(any)).thenAnswer(
        (_) async => Response(data: response, statusCode: 200),
      );
    ...
    test('fetchData should throw NoContentException on 204 status code',
        () async {
      when(_client.get(any)).thenAnswer(
          (_) async => Response(data: "https://http.cat/204", statusCode: 204));
    ...
    test(
      ...
      when(_client.get(any)).thenAnswer(
          (_) async => Response(data: "https://http.cat/400", statusCode: 400));
      ...
      when(_client.get(any)).thenAnswer((_) async =>
          Response(data: "<q:quakeml></q:quakeml>", statusCode: 400));
