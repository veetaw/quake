...
class FSDNews {
  ...
  Future<List<Earthquake>> fetchData({@required FSDNewsOptions options}) async {
    ...
    final Map _responseAsJson =
        await compute(_convertXmlToJson, _response.data.toString());
  ...
  String _buildUrl() =>
      (_apiUrl.scheme.isEmpty ? 'http' : _apiUrl.scheme) +
      _apiUrl.host +
      (_apiUrl.path.isEmpty ? '/fdsnws/event/1/query' : _apiUrl.path);
